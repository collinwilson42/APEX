...
"""
            ...
            print(f"[FAIL] Full analysis error: {e}")
            import traceback
            traceback.print_exc()
...
"""
            ...
            print(f"[FAIL] Lightweight update error: {e}")
            return self._safe_default()
...
class WebhookWriterV4:
    ...
    def _write_signal(self, signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(f"[OK] Webhook signal sent #{self.signals_sent}")
            print(f"  {json_str}")
            
            return True
            
        except Exception as e:
            print(f"[FAIL] Failed to write webhook signal: {e}")
            return False
...
def execute_decision(
    ...
    try:
        ...
        if delta == 0:
            print("[OK] Hold position - no action")
            return True
        ...
        if current_price is None:
            print("[FAIL] Cannot get current price")
            return False
        ...
        for i in range(abs_delta):
            if delta > 0:
                ...
                if current_volume >= config.MAX_TOTAL_POSITION:
                    print(f"[WARN]  Cannot increase: at max position")
                    break
                ...
                if abs(current_volume) < 0.1:
                    print("[WARN]  Cannot decrease: no position")
                    break
        ...
        print(f"[FAIL] Error executing decision: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def get_latest_screenshot(screenshot_folder: str) -> Optional[str]:
    ...
    try:
        if not os.path.exists(screenshot_folder):
            print(f"[WARN]  Screenshot folder not found: {screenshot_folder}")
            return None
        ...
        if not screenshots:
            print(f"[WARN]  No screenshots found in: {screenshot_folder}")
            return None
        ...
        print(f"[FAIL] Error finding latest screenshot: {e}")
        return None
    ...
    try:
        ...
        if need_full_analysis:
            ...
            if not screenshot_path:
                print(f"[WARN]  No screenshots available in {config.SCREENSHOT_FOLDER}")
                print(f"[WARN]  Skipping full analysis, will retry next interval")
                return False
...
def run_engine_cycle(
    ...
    try:
        ...
        if engine_state.countdown_remaining_sec <= 0:
            ...
            # WRITE TO BRIDGE - TUNITY SIGNAL FORMAT
            import json
            ...
            try:
                ...
                print(f"[OK] SIGNAL SENT TO SHEPHERD")
                print(f"  Timestamp: {signal['timestamp']}")
                print(f"  Action: {signal['action']} {signal['qty']} {signal['symbol']}")
                print(f"  Gate: {engine_state.peak_zone} | Score: {int(engine_state.current_score)}")
...
def main():
    ...
    print("[OK] Webhook writer V4 initialized")
    ...
    if config.API_ENABLED:
        try:
            ...
            print("[OK] Claude API V4 engine initialized")
            print("  - Full analysis (any Δ): Every 15 minutes")
            print("  - Lightweight (±1): Every 1 minute")
        except Exception as e:
            print(f"[WARN]  API disabled: {e}")
            if not os.getenv('ANTHROPIC_API_KEY'):
                print(f"[WARN]  No API key found in config or environment")
    else:
        print("[WARN]  API disabled via config (testing mode)")
    ...
    print(f"[OK] 15m: {snapshot_15m['timestamp']} | ${snapshot_15m['price_close']:.2f}")
