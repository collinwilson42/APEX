/* ═══════════════════════════════════════════════════════════════════════════
   DATABASE PANEL: XAUG26 (Gold Futures)
   Hardcoded endpoint mapping to XAUG26_intelligence.db
   ═══════════════════════════════════════════════════════════════════════════ */
...
const DB_XAUG26 = {
    ...
    SYMBOL_TICKER: 'XAUG26.sim',
    DB_PATH: 'XAUG26_intelligence.db',
    ...
    endpoints: {
        profiles: '/api/profiles?symbol=XAUG26',
        hyperspheres: '/api/hyperspheres?symbol=XAUG26',
        core: '/api/chart-data?symbol=XAUG26',
        basic: '/api/basic?symbol=XAUG26',
        advanced: '/api/advanced?symbol=XAUG26',
        fibonacci: '/api/fibonacci?symbol=XAUG26',
        ath: '/api/ath?symbol=XAUG26',
        stats: '/api/symbols'
    },
    ...
    init(containerId) {
        ...
        if (!this.container) {
            console.error('[DB_XAUG26] Container not found:', containerId);
            return;
        }
        this.render();
        this.loadData();
    ...
    render() {
        this.container.innerHTML = `
            <div class="database-split-panels" data-symbol="XAUG26">
                ...
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="xaug26-left-panel">
                ...
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="xaug26-right-panel">
                        <div class="database-loading">Loading...</div>
                    </div>
                </div>
            </div>
    ...
    initControls() {
        // Source tabs (Profiles / Hyperspheres)
    ...
    async loadProfiles() {
        try {
            ...
            console.warn('[DB_XAUG26] Failed to load profiles:', e.message);
            this.profiles = [];
        }
    },
    ...
    async loadHyperspheres() {
        try {
            ...
            console.warn('[DB_XAUG26] Failed to load hyperspheres:', e.message);
            this.hyperspheres = [];
        }
    },
    
    loadLeftPanel() {
        const container = document.getElementById('xaug26-left-panel');
        if (!container) return;
    ...
    async loadRightPanel() {
        const container = document.getElementById('xaug26-right-panel');
        if (!container) return;
        ...
        try {
            ...
            console.warn('[DB_XAUG26] Right panel error:', e.message);
