/* ═══════════════════════════════════════════════════════════════════════════
   DATABASE PANEL: US100Z25 (Nasdaq 100 Futures)
   Hardcoded endpoint mapping to US100Z25_intelligence.db
   ═══════════════════════════════════════════════════════════════════════════ */

const DB_US100Z25 = {
    SYMBOL_ID: 'US100Z25',
    SYMBOL_NAME: 'Nasdaq 100 Futures',
    SYMBOL_TICKER: 'US100Z25.sim',
    DB_PATH: 'US100Z25_intelligence.db',
    ...
    endpoints: {
        profiles: '/api/profiles?symbol=US100Z25',
        hyperspheres: '/api/hyperspheres?symbol=US100Z25',
        core: '/api/chart-data?symbol=US100Z25',
        basic: '/api/basic?symbol=US100Z25',
        advanced: '/api/advanced?symbol=US100Z25',
        fibonacci: '/api/fibonacci?symbol=US100Z25',
        ath: '/api/ath?symbol=US100Z25',
        stats: '/api/symbols'
    },
    
    init(containerId) {
        this.container = document.getElementById(containerId);
        if (!this.container) { console.error('[DB_US100Z25] Container not found:', containerId); return; }
        this.render();
        this.loadData();
    },
    ...
    render() {
        this.container.innerHTML = `
            <div class="database-split-panels" data-symbol="US100Z25">
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="us100z25-left-panel"><div class="database-loading">Loading...</div></div>
                </div>
                <div class="database-panel">
                    <div class="database-panel__header">
                        <div class="database-tab-group">
                    ...
                    <div class="database-panel__content" id="us100z25-right-panel"><div class="database-loading">Loading...</div></div>
                </div>
            </div>
        `;
        this.initControls();
    ...
    loadLeftPanel() {
        const c = document.getElementById('us100z25-left-panel'); if (!c) return;
        c.innerHTML = this.currentSource === 'profiles' ? this.renderProfilesTable() : this.renderHyperspheresTable();
    },
    
    async loadRightPanel() {
        const c = document.getElementById('us100z25-right-panel'); if (!c) return;
...
window.DB_US100Z25 = DB_US100Z25;
