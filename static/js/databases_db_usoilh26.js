/* ═══════════════════════════════════════════════════════════════════════════
   DATABASE PANEL: USOILH26 (Crude Oil Futures - March 2026)
   Hardcoded endpoint mapping to USOILH26_intelligence.db
   ═══════════════════════════════════════════════════════════════════════════ */

const DB_USOILH26 = {
    // Database Configuration
    SYMBOL_ID: 'USOILH26',
    SYMBOL_NAME: 'Crude Oil Futures',
    SYMBOL_TICKER: 'USOILH26.sim',
    DB_PATH: 'USOILH26_intelligence.db',
    ...
    endpoints: {
        profiles: '/api/profiles?symbol=USOILH26',
        hyperspheres: '/api/hyperspheres?symbol=USOILH26',
        core: '/api/chart-data?symbol=USOILH26',
        basic: '/api/basic?symbol=USOILH26',
        advanced: '/api/advanced?symbol=USOILH26',
        fibonacci: '/api/fibonacci?symbol=USOILH26',
        ath: '/api/ath?symbol=USOILH26',
        stats: '/api/symbols'
    },
    ...
    init(containerId) {
        ...
        if (!this.container) {
            console.error('[DB_USOILH26] Container not found:', containerId);
            return;
        }
        this.render();
        this.loadData();
    ...
    render() {
        this.container.innerHTML = `
            <div class="database-split-panels" data-symbol="USOILH26">
                ...
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="usoilh26-left-panel">
                ...
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="usoilh26-right-panel">
                        <div class="database-loading">Loading...</div>
                    </div>
                </div>
            </div>
    ...
    async loadProfiles() {
        try {
            ...
            console.warn('[DB_USOILH26] Failed to load profiles:', e.message);
            this.profiles = [];
        }
    },
    ...
    async loadHyperspheres() {
        try {
            ...
            console.warn('[DB_USOILH26] Failed to load hyperspheres:', e.message);
            this.hyperspheres = [];
        }
    },
    
    loadLeftPanel() {
        const container = document.getElementById('usoilh26-left-panel');
        if (!container) return;
    ...
    async loadRightPanel() {
        const container = document.getElementById('usoilh26-right-panel');
        if (!container) return;
        ...
        try {
            ...
            console.warn('[DB_USOILH26] Right panel error:', e.message);
...
window.DB_USOILH26 = DB_USOILH26;
