/* ═══════════════════════════════════════════════════════════════════════════
   DATABASE PANEL: BTCZ25 (Bitcoin Futures)
   Hardcoded endpoint mapping to BTCZ25_intelligence.db
   ═══════════════════════════════════════════════════════════════════════════ */

const DB_BTCZ25 = {
    SYMBOL_ID: 'BTCZ25',
    SYMBOL_NAME: 'Bitcoin Futures',
    SYMBOL_TICKER: 'BTCZ25.sim',
    DB_PATH: 'BTCZ25_intelligence.db',
    ...
    endpoints: {
        profiles: '/api/profiles?symbol=BTCZ25',
        hyperspheres: '/api/hyperspheres?symbol=BTCZ25',
        core: '/api/chart-data?symbol=BTCZ25',
        basic: '/api/basic?symbol=BTCZ25',
        advanced: '/api/advanced?symbol=BTCZ25',
        fibonacci: '/api/fibonacci?symbol=BTCZ25',
        ath: '/api/ath?symbol=BTCZ25',
        stats: '/api/symbols'
    },
    
    init(containerId) {
        this.container = document.getElementById(containerId);
        if (!this.container) { console.error('[DB_BTCZ25] Container not found:', containerId); return; }
        this.render();
        this.loadData();
    },
    ...
    render() {
        this.container.innerHTML = `
            <div class="database-split-panels" data-symbol="BTCZ25">
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="btcz25-left-panel"><div class="database-loading">Loading...</div></div>
                </div>
                <div class="database-panel">
                    <div class="database-panel__header">
                        <div class="database-tab-group">
                    ...
                    <div class="database-panel__content" id="btcz25-right-panel"><div class="database-loading">Loading...</div></div>
                </div>
            </div>
        `;
        this.initControls();
    ...
    loadLeftPanel() {
        const c = document.getElementById('btcz25-left-panel'); if (!c) return;
        c.innerHTML = this.currentSource === 'profiles' ? this.renderProfilesTable() : this.renderHyperspheresTable();
    },
    
    async loadRightPanel() {
        const c = document.getElementById('btcz25-right-panel'); if (!c) return;
