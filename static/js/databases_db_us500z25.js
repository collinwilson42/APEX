/* ═══════════════════════════════════════════════════════════════════════════
   DATABASE PANEL: US500Z25 (S&P 500 Futures)
   Hardcoded endpoint mapping to US500Z25_intelligence.db
   ═══════════════════════════════════════════════════════════════════════════ */

const DB_US500Z25 = {
    SYMBOL_ID: 'US500Z25',
    SYMBOL_NAME: 'S&P 500 Futures',
    SYMBOL_TICKER: 'US500Z25.sim',
    DB_PATH: 'US500Z25_intelligence.db',
    ...
    endpoints: {
        profiles: '/api/profiles?symbol=US500Z25',
        hyperspheres: '/api/hyperspheres?symbol=US500Z25',
        core: '/api/chart-data?symbol=US500Z25',
        basic: '/api/basic?symbol=US500Z25',
        advanced: '/api/advanced?symbol=US500Z25',
        fibonacci: '/api/fibonacci?symbol=US500Z25',
        ath: '/api/ath?symbol=US500Z25',
        stats: '/api/symbols'
    },
    
    init(containerId) {
        this.container = document.getElementById(containerId);
        if (!this.container) { console.error('[DB_US500Z25] Container not found:', containerId); return; }
        this.render();
        this.loadData();
    },
    ...
    render() {
        this.container.innerHTML = `
            <div class="database-split-panels" data-symbol="US500Z25">
                <div class="database-panel">
                    ...
                    <div class="database-panel__content" id="us500z25-left-panel"><div class="database-loading">Loading...</div></div>
                </div>
                <div class="database-panel">
                    <div class="database-panel__header">
                        <div class="database-tab-group">
                    ...
                    <div class="database-panel__content" id="us500z25-right-panel"><div class="database-loading">Loading...</div></div>
                </div>
            </div>
        `;
        this.initControls();
    ...
    loadLeftPanel() {
        const c = document.getElementById('us500z25-left-panel'); if (!c) return;
        c.innerHTML = this.currentSource === 'profiles' ? this.renderProfilesTable() : this.renderHyperspheresTable();
    },
    
    async loadRightPanel() {
        const c = document.getElementById('us500z25-right-panel'); if (!c) return;
...
window.DB_US500Z25 = DB_US500Z25;
