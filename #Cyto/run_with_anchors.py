...
NOW WITH HEXADIC ANCHORS on the 1.000 golden ratio line
"""
...
def init_db():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS anchors (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            anchor_string TEXT UNIQUE NOT NULL,
            version_major INTEGER,
            version_minor INTEGER,
            resonance_station INTEGER CHECK(resonance_station IN (1,2,4,5,7,8)),
            domain TEXT,
            alignment_station INTEGER CHECK(alignment_station IN (1,2,4,5,7,8)),
            confidence_station INTEGER CHECK(confidence_station IN (1,2,4,5,7,8)),
...
def save_anchor(anchor_data):
    """Save inline anchor to database."""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    try:
        cursor.execute("""
            INSERT INTO anchors (
                ...
                confidence_station, temporal_marker,
                theta, w_layer, source, timestamp
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
...
def get_all_anchors():
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM anchors ORDER BY created_at DESC")
...
def parse_anchor_string(anchor_str):
    """Parse inline anchor: v1.5 r5 d.RODIN a8 c8 t.NOW"""
...
# ============================================
# HTML - UPDATED WITH ANCHOR RENDERING
# ============================================
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        #anchorBadge .count { font-weight: bold; font-size: 1rem; }
        #viewInfo {
            position: absolute;
            top: 15px;
            right: 15px;
        ...
        .anchor-legend .title { font-size: 0.7rem; color: #C084FC; margin-bottom: 3px; }
...
<body tabindex="0">
    <div id="viewport">
        ...
        <div id="anchorBadge">
            ✨ Anchors: <span class="count" id="anchorCount">0</span>
        </div>
        <div id="viewInfo">
    ...
    <div id="panel">
        <div class="row">
            <div class="title">CYTO + Live Anchors</div>
            <div class="btns">
                <button class="btn green" onclick="testNodes()">+Nodes</button>
                <button class="btn purple" onclick="testAnchor()">+Test Anchor</button>
        ...
        <div class="stats">
            ...
            <div class="stat"><div class="lbl">Anchors</div><div class="val" id="sAnchors">0</div></div>
        </div>
        <div class="anchor-legend">
            <div class="title">Station Colors:</div>
            <span class="legend-item"><span class="legend-dot" style="background: #ADEBB3"></span>1-Unity</span>
            <span class="legend-item"><span class="legend-dot" style="background: #20B2AA"></span>2-Duality</span>
            <span class="legend-item"><span class="legend-dot" style="background: #6B8DD6"></span>4-Structure</span>
            <span class="legend-item"><span class="legend-dot" style="background: #C084FC"></span>5-Transform</span>
            <span class="legend-item"><span class="legend-dot" style="background: #D4AF37"></span>7-Mystery</span>
            <span class="legend-item"><span class="legend-dot" style="background: #F4E4C1"></span>8-Infinity</span>
        </div>
...
let nodes = [];
let anchors = [];  // NEW: Anchor storage
let zoom = 1;
...
const STATION_COLORS = {
    1: '#ADEBB3', 2: '#20B2AA', 4: '#6B8DD6',
    5: '#C084FC', 7: '#D4AF37', 8: '#F4E4C1'
};
...
window.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Control') {
        ...
        oText.textContent = 'Select Layer...';
    ...
    if (K.ctrl && e.key >= '1' && e.key <= '9') {
        ...
        if (K.first === null) {
            K.first = n;
            oText.textContent = 'W' + n;
            oSub.textContent = 'Release Ctrl to focus';
        }
        e.preventDefault();
        return;
    }
...
function render() {
    ...
    drawAnchors(cx, cy, unit);  // NEW: Draw anchors
...
// NEW: Draw anchors on the 1.000 golden ratio line
function drawAnchors(cx, cy, unit) {
    anchors.forEach(anchor => {
        if (!V.layers.includes(anchor.w_layer)) return;
        
        const g = geom(V.map[anchor.w_layer]);
        const mr = g.mid * unit;  // Anchors on 1.000 line
        
        const a = (anchor.theta - 90) * Math.PI / 180;
        const x = cx + Math.cos(a) * mr;
        const y = cy + Math.sin(a) * mr;
        
        // Get station color
        const color = STATION_COLORS[anchor.resonance_station] || '#FFFFFF';
        ...
        // Confidence pulse
        if (anchor.confidence_station >= 7) {
            ctx.beginPath();
...
socket.on('connect', () => { 
    ...
    socket.emit('get_anchors');  // NEW
    console.log('✓ Connected to CYTO');
});
...
socket.on('state', data => {
    ...
    document.getElementById('sAnchors').textContent = anchors.length;
...
// NEW: Anchor events
socket.on('anchors', data => { 
    anchors = data; 
    console.log(`✓ Loaded ${anchors.length} anchors`);
    render(); 
});

socket.on('anchor', anchor => { 
    anchors.push(anchor);
    console.log(`✨ New anchor: ${anchor.anchor_string} (Station ${anchor.resonance_station})`);
    render(); 
});

socket.on('anchor_added', data => {
    console.log('📡 Anchor broadcast received:', data);
    // Will be handled by normal anchor event
});
...
function testAnchor() {
    // Create random test anchor
    const stations = [1,2,4,5,7,8];
    const domains = ['TEST', 'CODE', 'DESIGN', 'FLOW', 'BREAKTHROUGH'];
    const temporals = ['NOW', 'SPRINT', 'BUILD', 'SYNTH'];
    
    const station = stations[Math.floor(Math.random() * stations.length)];
    const domain = domains[Math.floor(Math.random() * domains.length)];
    const temporal = temporals[Math.floor(Math.random() * temporals.length)];
    
    const anchor = `v1.${station} r${station} d.${domain} a${station} c${station} t.${temporal}`;
    
    fetch('/api/create_anchor', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({ anchor_string: anchor, source: 'test' })
    });
}

function hardReset() {
    if (confirm('Reset database? This clears all nodes, anchors, and starts a new epoch.')) {
        nodes = [];
        anchors = [];
        V.history = [];
...
# NEW: Anchor API
@app.route('/api/create_anchor', methods=['POST'])
def api_create_anchor():
    """Create inline anchor from API or WebSocket."""
    data = request.json
    anchor_string = data.get('anchor_string')
    
    if not anchor_string:
        return jsonify({'error': 'anchor_string required'}), 400
    
    parsed = parse_anchor_string(anchor_string)
    if not parsed:
        return jsonify({'error': 'Invalid anchor format'}), 400
    ...
    anchor_id = save_anchor(anchor_data)
    
    if anchor_id:
        # Broadcast to all clients
        socketio.emit('anchor', {**anchor_data, 'id': anchor_id})
        socketio.emit('anchor_added', anchor_data)  # For bridge compatibility
        return jsonify({'ok': True, 'anchor_id': anchor_id})
    else:
        return jsonify({'error': 'Anchor already exists'}), 409
...
@socketio.on('get_anchors')
def on_get_anchors():
    emit('anchors', get_all_anchors())

@socketio.on('anchor_created')
def on_anchor_created(data):
    """Receive anchor from bridge."""
    ...
    if anchor_id:
        # Broadcast to ALL clients
        socketio.emit('anchor', {**anchor_data, 'id': anchor_id}, broadcast=True)
        socketio.emit('anchor_added', anchor_data, broadcast=True)
        print(f"✨ Anchor created: {anchor_string} (Station {parsed['resonance_station']})")
...
if __name__ == '__main__':
    ...
    pos = engine.get_position()
    print("\n" + "="*60)
    print("CYTO - 4D Temporal Database + Live Inline Anchors")
    print("="*60)
    print(f"Epoch: {engine.epoch}")
    print(f"Running: {pos['total_hours']:.2f} hours")
    print(f"Current: W{pos['w']} at {pos['theta']:.1f}°")
    print("="*60)
    print("📡 WebSocket: Live anchor streaming active")
    print("✨ Anchors appear on 1.000 golden ratio line")
    print("🎨 Color-coded by station (1,2,4,5,7,8)")
    print("="*60)
