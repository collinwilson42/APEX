...
class MT5AdvancedCollector:
    def __init__(self, symbol: str, db_path: str, timeframes: list = ['1m', '15m']):
        ...
        self.mt5_timeframes = {'1m': mt5.TIMEFRAME_M1, '15m': mt5.TIMEFRAME_M15}
    ...
    def collect_and_save(self, timeframe_str):
        ...
        tf_mt5 = self.mt5_timeframes[timeframe_str]
        
        rates = mt5.copy_rates_from_pos(self.symbol, tf_mt5, 0, 1)
        ...
        try:
            ...
            # Verify advanced insert
            cursor.execute("SELECT COUNT(*) FROM advanced_indicators WHERE timestamp=? AND timeframe=? AND symbol=?", 
                          (timestamp, timeframe_str, self.symbol))
            ...
            status = f"CORE+BASIC+ADV({adv_count})" + ('+FIB' if fib_data else '') + ('+ATH' if ath_data else '')
            print(f"[{self.symbol}-{timeframe_str}] ✓ {timestamp} [{status}]")
